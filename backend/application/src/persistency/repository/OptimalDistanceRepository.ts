...
import {
    errorFunctionsSQLWrapper,
    errorFunctionsUnits,
} from "../../commons/errorFunctions.js";
...
export interface PunctualDistanceResult {
    thesisName: string;
    timestamp: number;
    x: number;
    y: number;
...
class OptimalDistanceRepository {
    ...
    async findThesisOptimalDistance(
        thesisId: number,
        timeFilterFrom: number,
        timeFilterTo: number,
        alghoritmViewFlag = false,
    ): Promise<OptimalDistanceResult[]> {
        const errorFunction =
            await this.WateringAlgorithmParams.findOne<WateringAlgorithmParamsModel>({
            ...
            UNION (
                SELECT
                    ...
                    ROUND(
                        (
                            SUM(
                                ${errorFunctionsSQLWrapper[errorFunction.errorFunction]("fd.value")}
                                * fd.weight
                            ) / SUM(fd.weight)
                        )::numeric,
                        6
        ...
        return await this.sequelize.query<OptimalDistanceResult>(
            queryString,
            {
    ...
    async findSectorOptimalDistance(
        sectorId: number,
        timeFilterFrom: number,
        timeFilterTo: number,
        alghoritmViewFlag = false,
        ...
        const queryString = `
            ...
            SELECT
                ...
                "valueType",
                "unit",
                SUM(value * thesis_weight) AS value
            FROM (
                SELECT 
                    wd.thesis_id,
                    wd.device_id as "deviceId",
    ...
    async findPunctualDistance(
        thesisId: number,
        timestamp: number,
    ): Promise<PunctualDistanceResult[]> {
        const errorFunction =
            await this.WateringAlgorithmParams.findOne<WateringAlgorithmParamsModel>({
        ...
        return await this.sequelize.query<PunctualDistanceResult>(
            queryString,
            {
                type: QueryTypes.SELECT,
                replacements: {
