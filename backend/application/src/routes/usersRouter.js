...
const usersRouter = ({ userService, authenticationService, authorizationService }) => {
    ...
    */
     ...
     * /users/permits:
     *   get:
     *     summary: Return user permissions on certain resource and service available on it
     *     tags: [Users]
     *     description: |
     *        Return the permission level of logged user indicating the role and list of service available on a certain resources, identified by type and id.
     *        If resource is not specified it return the maximum role user has on some resource.
     *     parameters:
     *       - in: query
     *         name: entityType
     *         schema:
    ...
    router.get('/permits', async (req, res) => {
        ...
        try {
            ...
            const permits = await authorizationService.getUserPermissions(userId, entity, id)

            return res.status(200).json(permits)
        } catch (error) {
            console.error(`Fail while retrieving user permission data caused by: ${error.message}`);
            return res.status(500).json({ error: 'Error while retrieving user permissions' });
        }
    })
